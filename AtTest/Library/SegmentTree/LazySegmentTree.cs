...
namespace AtTest.Library.SegmentTree
{
    class ABC179_F
    {
        ...
        class LazySegmentTree3<ValueType, LazyValueType>
        {
            ...
            // 遅延評価値を保持する値に反映させる処理
            Func<Segment, LazyValueType, ValueType> project;
            // 保持する値の畳み込み処理
            Func<Segment, Segment, ValueType> integrate;
            ...
            public struct Segment
            {
                ...
                // 元の配列で畳みこんでいる要素の左端のインデックス
                public int indexFromLeft;
                // 元の配列で畳みこんでいる要素の数
                public int length;

                public Segment(ValueType val, int indexFromLeft, int length)
                {
                    this.val = val;
            ...
            public LazySegmentTree3(int length,
                Func<LazyValueType, LazyValueType, LazyValueType> evaluate,
                Func<Segment, LazyValueType, ValueType> project,
                Func<Segment, Segment, ValueType> integrate, LazyValueType initValue, ValueType exValue)
            {
                totalLength = 1;
                while (totalLength < length)
                {
                ...
                haveLazy = new bool[totalLength * 2 - 1];
                valTree = new Segment[totalLength * 2 - 1];
                ...
                for (int i = 0; i < length; i++)
                {
                    ...
                    valTree[i + totalLength - 1] = new Segment(exValue, currentIndexFromLeft, currentNodeLength);
                    valTree[i + totalLength - 1].val = project(valTree[i + totalLength - 1], initValue);
                }
            ...
            /// <param name="top">現在見ているツリー中の要素がカバーする配列中の左端インデックス</param>
            /// <param name="last">現在見ているツリー中の要素がカバーする配列中の右端インデックス</param>
            Segment ScanQuery(int left, int right, int i, int top, int last)
            {
                if (left == top && right == last)
                {
                    return valTree[i];
                ...
                else
                {
                    ...
                    Segment leftNode = ScanQuery(left, half, i * 2 + 1, top, half);
                    Segment rightNode = ScanQuery(half + 1, right, i * 2 + 2, half + 1, last);
                    ValueType value = integrate(leftNode, rightNode);

                    return new Segment(value, leftNode.indexFromLeft, leftNode.length + rightNode.length);
                }
            }
