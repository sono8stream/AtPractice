...
namespace AtTest._600problems
{
    class ARC061_E
    {
        ...
        static void Method(string[] args)
        {
            ...
            int m = nm[1];
            int length = 2000000;
            var stations = new Dictionary<int, List<Edge>>[n];
            for(int i = 0; i < n; i++)
            {
                stations[i] = new Dictionary<int, List<Edge>>();
            }
            for(int i = 0; i < m; i++)
            {
                int[] pqc = ReadInts();
                ...
                int q = pqc[1] - 1;
                int c = pqc[2];
                if (!stations[p].ContainsKey(c))
                {
                    stations[p].Add(c, new List<Edge>());
                }
                if (!stations[q].ContainsKey(c))
                {
                    stations[q].Add(c, new List<Edge>());
                }
                stations[p][c].Add(new Edge(q, 0));
                stations[q][c].Add(new Edge(p, 0));
            }

            long dammy = 0;
            long[] costs = null;//Dijkstra(0, stations, out dammy);
            WriteLine(costs[n - 1]);
        }
