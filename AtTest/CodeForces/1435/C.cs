...
namespace AtTest.CodeForces._1435
{
    class C
    {
        ...
        static void Method(string[] args)
        {
            ...
            long max = bArray[n - 1] - array[5];
            long min = bArray[0] - array[5];
            long res = max - min;
            var que = new PriorityQueue<int[]>();
            for(int i = 0; i < n; i++)
            {
                que.Enqueue(bArray[i] - array[5], new int[2] { i, 5 });
            }
            while (que.Count >= n)
            {
                var pair = que.Dequeue();
                min = pair.Key;
                int idx = pair.Value[0];
                int aIdx = pair.Value[1];
                res = Min(res, max - min);
                if (aIdx > 0)
                {
                    max = Max(max, bArray[idx] - array[aIdx - 1]);
                    que.Enqueue(bArray[idx] - array[aIdx - 1], new int[2] { idx, aIdx - 1 });
                }
            }
